...
`
      ...
      <P>Upcasting is specifically a form of type casting wherein an object of a derived-class type is casted (converted) into its base-class type. However, the way that we'll use it, we won't actually be casting the "real" types of the objects<Emdash/>just their <It>annotated</It> types. This can mostly be done implicitly (i.e., without any special, explicit casting syntax). However, you do often need to explicitly type-annotate the variable in which the upcasted object(s) is / are stored.</P>

      <P>For example, a <Code>Zombie</Code> object can be implicitly type-casted into a <Code>Monster</Code> object (as can a <Code>Vampire</Code> object or a <Code>Goblin</Code> object). In case you don't realize how big of a deal this is, let me show you:</P>
