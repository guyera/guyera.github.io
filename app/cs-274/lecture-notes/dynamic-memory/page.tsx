...
`
      ...
      <P>Although occasional one-off memory leaks aren't a huge deal by themselves, they can make it more difficult to debug the more problematic iterative memory leaks that build up over time. Moreover, a one-off memory leak can become an iterative leak if the offending function is ever (one day) called inside a loop. Hence, you should make it your goal to avoid <Ul>all</Ul> memory leaks. Memory leaks of any kind may be penalized in some of this course's labs and assignments (always read the rubric!).</P>
