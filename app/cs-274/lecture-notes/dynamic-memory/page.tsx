...
`
      ...
      <P>I recognize that most students in this class don't have experience reading or writing assembly code. That's okay; let me walk you through it. <Code>%rbp</Code> is the register base pointer. It's the register that I was talking about that's conventionally used by compiled programs to keep track of the base address of the current function's stack frame (i.e., the top stack frame in the process's stack). Writing out the register in parentheses prefixed with a number will invoke some pointer arithmetic: it adds the prefixed number to the address stored in the register, and then it dereferences the result. For example, <Code>-4(%rbp)</Code> subtracts 4 from the base address of the current stack frame and then dereferences the result. <Code>-8(%rbp)</Code> subtracts 8 and dereferences the result. And so on. In other words, each number prefixing <Code>(%rbp)</Code> is likely a stack offset of some variable or another.</P>
      ...
      <P>These three steps clearly do <Ul>not</Ul> allocate and free data in a last-in-first-out (LIFO) fashion. They involve allocating a new block of memory, and then freeing an <It>older</It> block of memory shortly thereafter, without freeing the recently allocated new block first. Hence, this can't be done on any stacklike structure. Indeed, this highlights another limitation of the stack: the fact that it's managed automatically as a LIFO structure is often nice, but it means that you can't simply allocate and free data from arbitrary parts of the stack whenever you want.</P>

      <P>To summarize: the stack is compact, performant, and simple to use, but objects can't be freely moved around within the stack, and objects can't be freed from the stack at arbitrary times at your discretion. Pros and cons. This is all a result of how the stack is designed.</P>
