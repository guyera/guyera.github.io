...
`void foo(void) {
      ...
      <P>You can imagine a process's stack like a stack of plates. If you want to add a plate to the stack, you must put it on top. If you want to remove a plate from the stack, you must remove a plate from the top. Hence, the first plate to be removed is always the last plate that was added. It's not possible to remove a plate from the bottom or middle of the stack. A process's stack works the same way: bytes of memory can be allocated on top of the stack for some purpose, and bytes can be freed (deallocated) from the top of the stack, but that's it. It's not possible to free bytes from the bottom or middle of the process's stack. (Yes, this is a slightly weak analogy; you <It>could</It> shimmy a plate out from the middle of a stack of plates, but you fundamentally <Ul>cannot</Ul>, by definition, free bytes from the middle of a process's stack).</P>

      <P>This all happens automatically. Whenever the program needs to allocate some bytes of memory on the stack to store a new automatic variable, it does so automatically. And whenever the program needs to free one or more automatic variables (e.g., at the ends of their scopes or functions), it does so automatically. This marks a huge advantage of the stack: it has an incredibly simple interface. <It>You</It> don't have to think about freeing memory allocated on the stack.</P>

      <P>However, that simplicity comes at a cost in <It>capability</It>: because you don't have to decide when stack-allocated data gets freed, you don't <It>get</It> to decide when it gets freed. If you want some data to outlive the scope and / or function in which it was allocated, that's not an option on the stack.</P>
      ...
      <P>The simplicity of the stack's interface is <It>one</It> reason why data can't be moved around within it, but it's not the only reason. After all, in many other programming languages, there exist other memory models with simple, automatic interfaces that <It>do</It> support object movement (e.g., Python's heap). So what gives?</P>

      <P>Well, another reason that objects can't be moved around on the stack is that the stack was designed to optimize compactness and performance. This required further design decisions that, in combination with the stack's simple and automatic interface, preclude object movement.</P>

      <P>Let's discuss some of those design decisions.</P>

      <SectionHeading id="how-the-stack-works">How the stack works</SectionHeading>

      <P>Warning: lots of theory incoming.</P>

      <P>Automatic variables are specifically stored in <Bold>stack frames</Bold>. A stack frame is a block of bytes on the stack that's big enough to store all the automatic variables (and some other data, like function return addresses) used in an entire function, including all its nested scopes. Whenever a function starts, a stack frame is allocated for it (<Bold>pushed</Bold>) on top of the stack. Throughout the function, as individual variables are allocated and freed (e.g., as scopes start and end), all of that happens <It>within</It> the function's stack frame. When the entire function ends, its associated stack frame is freed (<Bold>popped</Bold>) from the top of the stack.</P>

      <P>(Some automatic variables declared in adjacent / "sibling" scopes within the same function may even reuse the same part of the function's stack frame. This is a common optimization performed by the compiler.)</P>

      <P>In most compiled programs, the CPU constantly keeps track of the base address of the current stack frame (i.e., the memory address of the bottom edge of the stack frame that's at the top of the stack) by storing it in some dedicated register (conventionally <Code>rbp</Code><Emdash/>the register base pointer<Emdash/>on the x86-64 architecture). Whenever a new stack frame is pushed onto the stack (i.e., whenever a new function starts), or whenever a stack frame is popped from the stack (i.e., whenever a function ends), the CPU updates the base address stored in that register.</P>

      <P>(There's another register that keeps track of the address of the <It>top</It> of the current stack frame (<Code>rsp</Code><Emdash/>the register stack pointer<Emdash/>on the x86-64 architecture). Both of these registers are needed because the top of a stack frame can shift dynamically throughout a function in special, limited cases, such as when an automatic VLA is declared, so the CPU must keep track of the location of the bottom <It>and</It> top of the current stack frame separately.)</P>

      <P>With all that in mind, in order to access an object on the stack, the program only needs to know its offset relative to the base address of its stack frame, which is referred to as the object's <Bold>stack offset</Bold>. So long as it knows an object's stack offset, it can access the object by simply adding that offset to the base address of the stack frame (which is usually stored in the CPU register that I just mentioned) and dereferencing the resulting address. If that sounds similar to how a program accesses an element in an array<Emdash/>adding an offset to a base address and dereferencing the result<Emdash/>that's because it is. This is the general strategy for accessing data in basically <It>any</It> contiguous structure (yes, the stack is contiguous; more on this shortly).</P>
...
`
      ...
      <P>As it turns out, you can tell <Code>gcc</Code> to compile the above program into assembly code (i.e., a human-readable version of machine instructions), but <It>not</It> assemble that assembly code into an executable, by supplying the <Code>-S</Code> flag. This produces <Code>simpleprogram.s</Code>, containing the compiled assembly code:</P>
...
`
      ...
      <P>I recognize that most students in this class don't have experience reading or writing assembly code. That's okay; let me walk you through it. <Code>%rbp</Code> is the register base pointer. It's the register that I was talking about that's conventionally used by compiled programs to keep track of the base address of the current function's stack frame. Writing out the register in parentheses prefixed with a number will invoke some pointer arithmetic: it adds the prefixed number to the address stored in the register, and then it dereferences the result. For example, <Code>-4(%rbp)</Code> subtracts 4 from the base address of the current stack frame and then dereferences the result. <Code>-8(%rbp)</Code> subtracts 8 and dereferences the result. And so on. In other words, each number prefixing <Code>(%rbp)</Code> is likely a stack offset of some variable or another.</P>
      ...
      <P>This stack organization is highly performant in that it makes data on the stack extremely accessible to the CPU. Any instruction that needs to access a variable on the stack contains the offset of said variable directly within it, so the CPU gets this offset "for free" when it loads the instruction. Moreover, this stack organization allows the stack to remain contiguous at all times. The compiler decides exactly where all the automatic variables fit together within a given function's stack frame(s), and it leaves no gaps between them (except perhaps for performance reasons, such as byte alignment). When stack frames are actually created at runtime, they're pushed directly on top of the existing top stack frame, again with no gaps between them. No gaps means contiguity.</P>
      ...
      <P>So, these design decisions make the stack compact and highly performant. However, they create another reason why objects can't be moved freely from one part of the stack to another: the stack offset of each automatic variable is a compile-time constant. It's determined by the compiler, at compile time, and embedded directly into machine instructions in the compiled executable (e.g., -4, -8, and -12: the decided stack offsets of <Code>a</Code>, <Code>b</Code>, and <Code>c</Code> in <Code>simpleprogram.c</Code>). This means that stack offsets cannot be modified at runtime, hence objects cannot be moved around within their stack frames at runtime.</P>

      <SectionHeading id="dynamic-storage-duration-and-the-heap">Dynamic storage duration and the heap</SectionHeading>
      
      <P>To summarize what we've covered so far: the stack is designed to optimize compactness, performance, and simplicty of use, but that design sacrifices certain capabilities. Namely, objects can't be freely moved around within the stack, and, more generally, objects can't be freed from the stack at arbitrary times at your discretion. Pros and cons.</P>
      ...
      <P>Perhaps there exists some other place in memory where arrays can be created as well. Perhaps that other place isn't a stack at all, but rather has its own structure and follows its own ruleset. Perhaps that structure and ruleset support movement of objects, maybe at the cost of some compactness, performance, and / or ease of use. If that were the case, then we could create arrays <It>there</It> instead, and move them around when needed to facilitate their resizing.</P>
...
`
      ...
      <P>While the above program can theoretically be written to use an automatic VLA, this dynamic array solution avoids all the technical issues of VLAs. Moreover, I'll show you how to resize a dynamic array pretty soon<Emdash/>something that even VLAs can't do.</P>
      ...
      <P>However, seemingly innocuous one-off memory leaks can make it more difficult to debug the more problematic iterative memory leaks that build up over time (e.g., Valgrind can't distinguish them from one another). Moreover, a one-off memory leak can become an iterative leak if the offending function is ever called inside a loop. As a general rule of thumb, you should make it your goal to avoid all memory leaks. Memory leaks of any kind may be penalized in some of this course's labs and assignments (always read the rubric!).</P>
...
`
      ...
      <P>Now that we've found our leaked memory, we can free it. To free dynamic memory, call the <Code>free</Code> function, provided by <Code>stdlib.h</Code>. As the argument, provide a pointer storing the base address of the dynamically allocated block that you'd like to free. Yes, this is the exact same address that was returned by <Code>malloc</Code> and stored in <Code>numbers</Code> when the block was allocated. In other words:</P>
