...
`
      ...
      <P>Although occasional one-off memory leaks aren't a huge deal by themselves, they can make it far more difficult to debug the more problematic iterative memory leaks. Hence, you should make it your goal to avoid <Ul>all</Ul> memory leaks. Memory leaks of any kind may be penalized in some of this course's labs and assignments (always read the rubric!).</P>

      <P>Valgrind will tell you when you have memory leaks. When the program ends, Valgrind will print out a "HEAP SUMMARY". In the above Valgrind output, it says 688 bytes in 1 block were in use at exit. Anything other than "0 bytes in 0 blocks" means you have a memory leak. (688 bytes in particular is 172 <Code>float</Code>'s * 4 bytes per <Code>float</Code>).</P>
      ...
      <P>So, we have to fix our memory leak. In order to fix a memory leak, you must first <It>find</It> it. In this case, it's simple: the dynamic array that <Code>numbers</Code> points to was never freed. But in more complicated programs that allocate and free a lot of dynamic memory, it might not be obvious where the memory leak(s) are coming from. To locate memory leaks in a C program:</P>
