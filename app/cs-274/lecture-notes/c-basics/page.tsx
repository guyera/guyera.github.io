...
`
      ...
      <P><Code>stdio.h</Code> is a part of the C standard library, and it provides prototypes and definitions necessary to interface with standard input and output (standard I/O; <Link href="#standard-output">more on this in a bit</Link>). For example, the <Code>printf()</Code> function's prototype is provided by <Code>stdio.h</Code>. <Code>printf()</Code> is used within <Code>hello.c</Code>, so it must include <Code>stdio.h</Code> in order to get access to it.</P>
...
`
      ...
      <P>(Technically, the entire statement <Code>x = y + z</Code> is <It>also</It>, itself, an expression; it's a piece of code with a type and a value. <Link href="#automatic-variables">More on that shortly</Link>.)</P>

      <P>A statement may also be a function call, such as <Code>printf("Hello, World!\n");</Code>.</P>

      <P>In some cases, a group of several statements can be referred to as a statement as well. For example, an if statement is a group of statements that execute if and only if a certain condition is satisfied. <Link href="#if-statements">More on if statements later</Link>.</P>
...
`printf("%lf\\n", (double) (x / y)); // Prints 0.0`
      ...
      <P>Be careful when performing type-casting, especially explicit type-casting. In some cases, the compiler will let you type-cast an expression to another type even when the conversion doesn't make any sense. This can sometimes lead to undefined behavior, especially when casting between types of pointers, which we'll cover in <Link href={`${PARENT_PATH}/${allPathData["pointers"].pathName}`}>a future lecture</Link>.</P>
