...
async function LectureNotes({ allPathData }: { allPathData: any }) {
  return (
    <>
      ...
      <P>(I've left out several details. For example, besides the linker, there's also the <Bold>loader</Bold>. Or, as an alternative terminology, the linker is sometimes referred to more precisely as the "static linker" while the loader is referred to as the "dynamic linker". The linker (static linker) performs all the above steps that I mentioned, but it can leave some symbol relocation "incomplete", specifically for symbols defined in shared ("dynamically linked") libraries. For such symbols, it leaves placeholders for their eventual addresses in the process's Global Offset Table (GOT). At runtime (e.g., just as the program is first starting), the loader (dynamic linker) maps the shared physical memory on the system containing those symbols' definitions into the process's virtual address space, and then, if necessary, it updates the corresponding GOT entries to point to those mapped virtual addresses. This allows several processes that depend on the same shared library to share its external symbol definitions, avoiding, say, the same library's machine instructions (code) from having to be duplicated in physical memory for each running process that depends on it. By default, the C standard library is usually dynamically linked in this manner. This means that the C standard library's code is not technically contained within every C program's binary, nor duplicated in physical memory for each running C program.)</P>
