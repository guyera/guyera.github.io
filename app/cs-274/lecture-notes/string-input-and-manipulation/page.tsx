...
ssize_t len = getline(&line, &n, stdin);`
      ...
      <Itemize>
        <Item><P><Code>line</Code> will store the base address of a brand new <Ul>dynamically allocated</Ul> character array containing a (null-terminated) C string representing an entire line of text that was read from standard input (e.g., supplied by the user).</P></Item>
        <Item><P><Code>n</Code> will store the size (number of bytes; equivalently, number of characters) of the dynamically allocated character array that <Code>line</Code> now points to.</P></Item>
        <Item><P><Code>len</Code> (the stored return value) will store the length of the <Ul>contents</Ul> of the C string that <Code>line</Code> now points to (i.e., the number of characters in the line of text entered by the user, not including the injected null terminator). This will always be strictly less than <Code>n</Code>. This saves you a call to <Code>strlen</Code> if you subsequently need to know the length of the string (and you often do).</P></Item>
      </Itemize>
...
free(copy);`
      ...
      <Enumerate listStyleType="decimal">
        <Item><P>If the first argument is not <Code>NULL</Code>, then it must point to the beginning of a C string to be tokenized. In such a case, the token extraction process will start from the beginning of said string. Otherwise, if it <It>is</It> <Code>NULL</Code>, then the token extraction process will pick up from the character where it left off in the previous call to <Code>strtok_r</Code>. The address of this character must be stored in <Code>saveptr</Code> (i.e., the <Code>char*</Code> variable that the third argument points to).</P></Item>
        <Item><P>It searches the string (starting from the beginning, or from where it left off in a previous call, depending on the values of the first and third arguments) for a token separator. The list of token separators to be considered is specified as a C string via the second argument to <Code>strtok_r</Code> (<Code>","</Code> in the above demo).</P></Item>
        <Item><P>If it finds a token separator, it <Ul>modifies</Ul> it, changing it into a null terminator. It then updates <Code>saveptr</Code> (the <Code>char*</Code> variable that the third argument points to) to store the address of the character that appears immediately <It>after</It> that token separator.</P></Item>
        <Item><P>It returns the address of the character from where it started the token extraction process (i.e., the beginning of the extracted token).</P></Item>
      </Enumerate>
